...
export function AdminImportarLinksDialog() {
    ...
    Papa.parse<CSVRow>(file, {
      ...
      complete: async (parseResult) => {
        ...
        for (let i = 0; i < rows.length; i++) {
          ...
          try {
            if (!row.titulo || !row.url_zoom) {
              importResults.push({
                titulo: row.titulo || `Linha ${i + 1}`,
                status: "error",
                message: "Campos obrigatórios faltando (titulo, url_zoom)",
              });
              continue;
            }
        ...
        toast({
          ...
          description: `${successCount} links criados, ${errorCount} erros`,
        });
      },
      error: (error) => {
        toast({
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Importar Links via CSV</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              O arquivo CSV deve conter as colunas: <strong>titulo, url_zoom</strong>
            </AlertDescription>
          </Alert>
